...
/* int uFMOD_DSPlaySong(
  ...
  Return Values:
  ---
     Returns a positive value on success or -1 otherwise. The return
     value can be handled as a COM HRESULT.
  ---
  Remarks:
  ---
     If no valid song is specified and there is one currently being
     played, uFMOD_DSPlaySong just stops playback.
...
/* void uFMOD_Pause(void)
   ...
   ---
      ...
      IDirectSoundBuffer::Stop/Play functions.
*/
...
/* void uFMOD_SetVolume(
   ...
   ---
      ...
      uFMOD_DSPlaySong calls. You can set the desired volume level before
      actually starting to play a song.
      You can use DirectSound and KMixer functions directly to control the L
      and R channels volumes separately. KMixer also has a wider range than
      ...
      things about KMixer is that it may produce clicks and is hardware
      dependent.
*/
...
#define uFMOD_BUFFER_SIZE 0x10000
